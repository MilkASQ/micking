<template>
  <main class="rechargeLog">
    <vhead title="充值记录" :url="{name:'UC_WITHDRAW'}" rightUrl="" status="false" rightTxt=""></vhead>
      ...
      <section class="moneyLog" v-if="list.length > 0" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading"
        ...
        <ul>
          <li v-for="item in list">
            <div class="left">
              <p class="money">金额：<em>{{item.money}}元</em></p>
              <p class="time">{{item.time}}</p>
            </div>
            <div class="right">
              <!--<div class="msg">{{item.msg}}</div>-->
              <button type="button" class="submit" v-if="item.status === 2">撤销提现</button>
              <div class="status" :class="{red: item.status === 3 , blue: item.status === 1 }" v-else>
                {{item.status == 1 ? '提现成功' : '提现失败'}}
              </div>
            </div>
          </li>
        </ul>
